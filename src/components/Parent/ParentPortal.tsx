...
export const ParentPortal: React.FC<ParentPortalProps> = ({
  ...
  if (!alumno) {
    return (
      <div className="flex h-full flex-col items-center justify-center bg-canvas p-4 text-ink animate-fadeIn">
        <div className="w-full max-w-md rounded-2xl border border-line bg-surface p-6 sm:p-8 shadow-2xl space-y-6">
          <div className="text-center space-y-2">
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 text-3xl border border-primary/25 text-primary shadow-inner">
              🎒
            </div>
            <h2 className="text-xl sm:text-2xl font-black text-ink">Portal del Representante</h2>
            <p className="text-xs sm:text-sm text-muted leading-relaxed">
              Ingresa el <b className="text-primary">ID del Alumno</b> para consultar el estado del transporte escolar y el recorrido en tiempo real.
            </p>
          </div>
          ...
          >
            <div>
              <label htmlFor="input-student-id" className="block text-xs font-bold uppercase tracking-wider text-ink mb-1.5">
                ID del Alumno *
              </label>
            ...
            <button
              id="btn-parent-login"
              type="submit"
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-primary py-3 text-sm font-black text-ink hover:bg-primary active:scale-95 transition-all shadow-lg cursor-pointer"
            >
            ...
            <div className="flex flex-wrap gap-1.5">
              {allStudents.slice(0, 5).map((s) => (
                <button
                  ...
                  className="rounded-lg bg-soft-gray border border-line px-2.5 py-1 text-xs text-ink hover:border-primary hover:text-primary transition-all font-mono cursor-pointer"
                >
  ...
  const getStudentStatus = () => {
    if (ruta.estado === 'completada' || parada?.estado === 'completado') {
      return {
        ...
        badgeColor: 'bg-emerald-500 text-ink',
    ...
    if (ruta.estado === 'en_curso') {
      ...
      return {
        ...
        badgeColor: isNext ? 'bg-primary text-white animate-pulse' : 'bg-primary/10 text-primary',
    ...
    return {
      ...
      badgeColor: 'bg-soft-gray text-ink border border-line',
  ...
  return (
    <div className="flex h-full flex-col bg-canvas text-ink animate-fadeIn">
      {/* Top Parent Header */}
      <div className="border-b border-line bg-surface px-4 py-3 shadow-md">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-xl border border-primary/30 text-primary">
              🎒
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="font-bold text-sm sm:text-base text-ink">{alumno.nombre}</h2>
                <span className="rounded-md bg-primary/10 text-primary border border-primary/25 px-1.5 py-0.2 text-[10px] font-mono font-bold">
                  ID: {alumno.id}
                </span>
              </div>
              <p className="text-xs text-muted">
                Representante: <span className="text-ink font-medium">{alumno.representante?.nombre || 'Titular'}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-soft-gray px-2.5 py-1.5 text-xs font-semibold text-ink border border-line hover:bg-line active:scale-95 transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-soft-gray px-2.5 py-1.5 text-xs font-semibold text-alert border border-line hover:bg-line transition-all cursor-pointer"
            >
        ...
        {allStudents.length > 1 && (
          <div className="mt-2.5 flex items-center gap-1.5 overflow-x-auto pb-1 text-xs">
            <span className="text-[11px] text-muted shrink-0">Cambiar alumno rápido:</span>
            {allStudents.map((s) => (
              <button
                key={s.id}
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 sm:space-y-4">
        {/* HERO STATUS CARD */}
        <div className="relative overflow-hidden rounded-2xl border border-line bg-gradient-to-b from-slate-900 to-slate-950 p-4 sm:p-5 shadow-xl">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-soft-gray text-2xl border border-line shadow-inner">
                {status.icon}
              </div>
              <div>
                ...
                <h3 className="text-base sm:text-lg font-black text-ink">{status.title}</h3>
              </div>
            </div>

            {parada && (
              <div className="text-right shrink-0">
                <span className="text-[10px] font-bold uppercase text-muted block">Hora Recogida</span>
          ...
          <p className="mt-2.5 text-xs sm:text-sm text-ink leading-relaxed">
            {status.description}
          </p>

          {/* Live Proximity Metric */}
          {distanceToStudentKm !== null && ruta.estado === 'en_curso' && parada?.estado === 'pendiente' && (
            <div className="mt-3 flex items-center justify-between rounded-xl bg-primary/10 border border-primary/25 p-2.5 text-xs text-primary">
              <div className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {/* Pickup Address Card */}
          <div className="rounded-xl border border-line bg-surface p-3.5 space-y-1.5">
            <span className="text-[10px] uppercase font-bold text-muted flex items-center gap-1">
              <MapPin className="h-3.5 w-3.5 text-primary" />
              Dirección de Recogida
            </span>
            <p className="text-xs font-semibold text-ink">{alumno.direccion_recogida}</p>
          ...
          <div className="rounded-xl border border-line bg-surface p-3.5 space-y-1.5">
            ...
            <p className="text-xs font-semibold text-ink">{colegio.nombre}</p>
        ...
        <div className="rounded-xl bg-surface border border-line p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
          ...
          <div className="flex items-center gap-2">
            {ruta.conductor?.telefono && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-1.5 rounded-xl bg-emerald-500/20 border border-emerald-200 px-3 py-2 text-xs font-bold text-emerald-600 hover:bg-emerald-500/30 transition-all shrink-0"
              >
                <span>WhatsApp</span>
              </a>
            )}
            <a
              ...
              className="flex items-center gap-1.5 rounded-xl bg-primary px-3.5 py-2 text-xs font-black text-ink shadow-md hover:bg-primary transition-all shrink-0"
            >
