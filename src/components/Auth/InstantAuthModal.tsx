...
export const InstantAuthModal: React.FC<InstantAuthModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-md animate-fadeIn">
      <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl space-y-4 text-slate-100 relative">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-black text-slate-100 text-base">Autenticación & InstantDB</h3>
        ...
        {/* Authenticated State */}
        {activeUser ? (
          <div className="rounded-xl border border-slate-800 bg-slate-950/80 p-4 space-y-4 text-xs">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="h-9 w-9 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-black border border-emerald-500/30">
                  <UserCheck className="h-5 w-5" />
                </div>
                <div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-slate-100 font-bold">{activeUser.email}</span>
                    <span className="rounded bg-amber-500/20 text-amber-300 px-1.5 py-0.2 text-[9px] font-black uppercase border border-amber-500/30">
                      {('rol' in activeUser && activeUser.rol) ? activeUser.rol : 'Admin'}
                    </span>
                  </div>
                  <span className="text-slate-400 text-[10px]">Sesión Activa de Administrador</span>
                </div>
              </div>
              <span className="rounded-md bg-emerald-500/20 text-emerald-400 px-2 py-0.5 text-[10px] font-bold border border-emerald-500/30">
                Conectado
              </span>
            </div>

            <div className="text-slate-400 text-[11px] border-t border-slate-800/80 pt-2 space-y-1">
              <p className="text-slate-300 font-medium">✓ Permisos totales para gestión de rutas, escuelas y estudiantes.</p>
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 rounded-xl bg-slate-800 py-2.5 text-xs font-bold text-rose-400 hover:bg-slate-700 transition-all cursor-pointer"
              >
          ...
          <div className="space-y-3.5">
            ...
            <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-3 space-y-2">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 rounded-lg bg-amber-500 py-2 text-xs font-black text-slate-950 hover:bg-amber-400 active:scale-95 transition-all shadow-md cursor-pointer"
              >
            ...
            <form onSubmit={handleSubmitLogin} className="space-y-3">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl bg-slate-950 border border-slate-700 pl-9 pr-3 py-2 text-xs text-slate-100 placeholder:text-slate-600 focus:border-amber-500 focus:outline-none"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 rounded-xl bg-slate-800 border border-slate-700 py-2.5 text-xs font-bold text-slate-100 hover:bg-slate-700 active:scale-95 transition-all disabled:opacity-50 cursor-pointer shadow-md"
              >
        ...
        <div className="border-t border-slate-800 pt-3 space-y-2">
          ...
          <button
            ...
            disabled={isSeeding}
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-slate-800 border border-slate-700 py-2 text-xs font-bold text-slate-200 hover:bg-slate-700 transition-all disabled:opacity-50 cursor-pointer"
          >
            <RefreshCw className={`h-3.5 w-3.5 text-amber-400 ${isSeeding ? 'animate-spin' : ''}`} />
