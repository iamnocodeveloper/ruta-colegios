...
- **Login real por rol** (`LoginGateway` + `App.handleStaffLogin`): el **dueño** entra con `admin@demo.com` / `123456` (superadmin, restaurado por requerimiento — el Magic Code va a un correo no controlado); el resto de usuarios entra por **InstantDB Magic Code** y resuelve rol (`superadmin`/`admin`/`conductor`) + cliente desde `usuarios.email` o `conductores.email`.
